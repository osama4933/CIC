...
DC = conj(sym_to_data_ang([1],N));	% generate standard downchirp symbol
DC_fft = fft(DC);
DC_upsamp =(ifft([DC_fft(1:N / 2) zeros(1,(upsampling_factor - 1) * N) DC_fft(N / 2 + 1:N)])); % upsample downchirp with sinc-interpolation
...
for i = 1:floor(length(x_1) / win_jump) - win_jump_factor
    % determined peak gain for each window
